...
│   ├── audio.js         # 本地 MP3 BGM + WebAudio 音效与故障回退
│   ├── core/            # 核心逻辑（纯算法，无平台依赖）
...
`res/audio/sfx-acoustic.mp3`含Mixkit Fairy arcade sparkle，已从公开Git源码中排除，但必须包含在本地微信发布包。不要强制`git add -f`或把原音/派生试听/混合精灵放到其他公开路径。仅克隆源码不会得到这份发布音效；须由项目持有人从合法本地副本恢复，不能把读取失败的合成回退当成正式音频验收通过。当前资源及复现输入见`../assets/_incoming/selected-sfx-20260915/README.md`；发布前运行音效、素材和包体检查。此限制不影响本机上传微信游戏终端作品，未授权任何本轮实际上传。
...
- 当前回归容差为变难不超过 12、变易不超过 24 个百分点；该模拟只用于工程标定，不代表真人通过率。
...
- 音频：用户确认的 ElevenLabs 原创本地 MP3 循环（舒缓/PvP）+ 21 cue 混合授权音效精灵 + WebAudio 故障回退；消除/横竖触发/通关采用 Mixkit，冰裂与炸弹采用 CC0，四连生成音采用 CC BY 3.0；音乐与音效可独立关闭
- 果冻（消除破层，棋子保留）与冰块（相邻波及融化）障碍
- 特殊棋子：横/竖四连→对应火箭、直线五连→彩球、T/L交叉→炸弹；支持特殊交换组合与波及连锁
...
当前音频：横/竖特殊触发采用Mixkit Magic wand sparkle，通关采用Christmas reveal tones；四枚成线生成音、冰裂、升调连消保持。21cue精灵61,022字节，含Mixkit的运行MP3只保留本地发布副本，不随公开源码分发。新两项声音真机验收仍需完成；来源见THIRD_PARTY_NOTICES与音频运行规范。

...
- 当前分数仍由客户端上报，服务端只校验单调性、单次增量和时间上限；当前也未对建房动作做按 OpenID 限频。对休闲好友局可作为首发残余风险，若增加排行、奖池或可交易价值，必须先升级服务端权威校验和限频。

## 上线门槛

1. 在微信云开发测试环境部署 `battle` 云函数，确认 SDK 4.0.2 的事务行为、`createdAt` 查询索引和权限规则；确认 `cleanup-battle-rooms` 定时触发器已创建并能删除 7 天前的测试房间。
...
- 待确认：定时触发器时区与实际执行日志、7 天测试房间清理结果、云函数错误/调用量告警和腾讯云费用预算提醒。

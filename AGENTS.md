...
- 可靠性冻结规则：全局错误只记录类别和计数；微信原生事件/监控上报默认关闭且必须配置显式映射；版本更新仅在非对局安全场景提示；隐私说明按2026-09-22用户授权优先调用 `wx.openPrivacyContract`，接口缺失或失败时回退随包本地长文页，正文与确认状态维护于 `js/core/privacy-policy.js`，发布前需运营方确认且后台声明保持一致。打开官方协议或阅读本地页均不代表同意，不替代必要的隐私授权；迟到回调不得打断离开设置后的操作。

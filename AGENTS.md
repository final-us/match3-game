...
- `match3-wechat/levels/`：20 个数据驱动关卡。
...
- 视觉冻结规则：全核心页面属于同一“月夜猫咪花园”世界；首页级高精度静态 3D 质感，动态特效克制；普通棋子使用统一高清猫咪透明素材，特殊棋子以猫咪底图叠加 Canvas 标记；鸿蒙继续降 DPR 并关闭碎屑粒子。
- 音频冻结规则：使用零版权风险的 WebAudio 程序化合成；舒缓场景与 PvP 使用两套月夜音乐盒旋律；音乐和音效独立控制；后台及音频中断时停止音乐调度。
